#!/usr/bin/env python3
#    _     _____    ________      _____         __      ________     ______     _     ___________     ______
#   |_|   ____      ______       |             /  \        __       |          |_|     _________     |
#   |_|  __         ____         |            /    \       __       |          |_|      _______      | 
#   |_|  __         _______         o        /      \      __          O       |_|       _____          O
#   |_|   ____      ________     |          /        \     __       |          |_|        ___        |
#   |_|    _____    __________   |______   /__________\    __       |______    |_|         _         |_______
#                                    
#                                   
#
#
...
configFile = installDir + "/icreative.cfg"
print(installDir)
config = ConfigParser.RawConfigParser()
config.read(configFile)

toolDir = installDir + config.get('icreative', 'toolDir')
logDir = installDir + config.get('icreative', 'logDir')
yes = config.get('icreative', 'yes').split()
color_random=[color.HEADER,color.IMPORTANT,color.NOTICE,color.OKBLUE,color.OKGREEN,color.WARNING,color.RED,color.END,color.UNDERLINE,color.LOGGING]
random.shuffle(color_random)
fsocietylogo = color_random[0] + '''
        d8       8888bd8   888d888   8888888         88       8bd88888b   88   88888bdbdb888   88888888
        88'     88YP8P     Y8d8P     88            88  88        88       8b    88888888888    88
        88     ooo8        bo88                   8P    88       88       oo     ooo888bd8     
        88     Y8b8        88888                 88      88      88       88      8888888      
        88      db8D8b     d8Y8b8    88         88        88     88       88       88888       88
        YP       8888YY8   8PY88P88  8888888   88888P88888888    YP       88        888        88888888
        '''
icreativePrompt = "icreative ~: "
alreadyInstalled = "Already Installed"
continuePrompt = "\nClick [Return] to continue"

termsAndConditions = color.NOTICE + '''
I shall not use icreative to:
...
As we all know, icreative co just launched a cyber security firm.

We will update to python 3.7 & add all of the new hacking tool of 4.0 later this year.


Thank you for all the support over the years, the icreative team thanks you!
...
'''
def agreement():
    while not config.getboolean("icreative", "agreement"):
        clearScr()
        print(termsAndConditions)
        print(mrrobot4)
        agree = raw_input("Agree to our terms and conditions first (Y/n) ").lower()
        if agree in yes:
            config.set('icreative', 'agreement', 'true')

class icreative:
    def __init__(self):
        clearScr()
        self.createFolders()
        print (icreativelogo + color.RED + '''
       }--------------{+} Coded By DaveAce {+}--------------{
       }--------{+}  GitHub.com/icreative/icreative {+}--------{
    ''' + color.END + '''
       ...
       {99}-EXIT\n
     ''')
        choice = raw_input(icreativePrompt)
        clearScr()
        if choice == "1":
    ...
    Yb      Yb   dP 88 Y88   88   88"Yb  88 88""Yb o.`Y8b
     ...
     ''')
        contributorsURL = 'https://api.github.com/repos/icreative-co/icreative/contributors'
    ...
    def update(self):
        os.system("git clone --depth=1 https://github.com/icreative-co/icreative.git")
        os.system("cd icreative && bash ./update.sh")
        os.system("icreative")
...
class sniffingSpoofingMenu:
    ...
    def __init__(self):
        ...
        choice6 = raw_input(icreativePrompt)
        clearScr()
        if choice6 == "1":
            setoolkit()
        elif choice6 == "2":
...
class webHackingMenu:
    ...
    def __init__(self):
        ...
        print("   {99}-Back To Main Menu \n")
        choiceweb = raw_input(icreativePrompt)
        clearScr()
        if choiceweb == "1":
            maine()
        elif choiceweb == "2":
...
class postExploitationMenu:
    ...
    def __init__(self):
        ...
        choice11 = raw_input(icreativePrompt)
        clearScr()
        if choice11 == "1":
            sitechecker()
        elif choice11 == "2":
...
class informationGatheringMenu:
    ...
    def __init__(self):
        ...
        print("  {8}--Crips\n  ")
        print("  {99}-Back To Main Menu \n")
        choice2 = raw_input(icreativePrompt)
        clearScr()
        if choice2 == "1":
            nmap()
        elif choice2 == "2":
...
class wirelessTestingMenu:
    ...
    def __init__(self):
        ...
        choice4 = raw_input(icreativePrompt)
        clearScr()
        if choice4 == "1":
            reaver()
        elif choice4 == "2":
...
class exploitationToolsMenu:
    ...
    def __init__(self):
        ...
        choice5 = raw_input(icreativePrompt)
        clearScr()
        if choice5 == "1":
            atscan()
        elif choice5 == "2":
